...
def take_measurements():
    ...
    # patient_waveform_raw = poll_sensor(sample_period.get(), patient_name.get())
    patient_waveform_raw = poll_for(sample_period.get(), patient_name.get())
    # patient_waveform = pulse_analysis(patient_waveform_raw)
    patient_pulse_data = pulse_analysis(patient_waveform_raw)
...
def refresh_plot():
    ...
    a.axis([int(s_idx[s]), int(e_idx[e]), min(patient_waveform[start_idx:end_idx]), max(patient_waveform[start_idx:end_idx])])
...
load_plot_button.pack(side=TOP)
