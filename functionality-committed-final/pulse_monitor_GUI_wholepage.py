...
def submit_name_button_func():
    ...
    if patient_name.get() == "Enter Patient Name":
        ...
        canvas.draw()
        # Update Patient Data
        patient_name_value.config(text=patient_name.get())
...
def take_measurements():
    ...
    # Set axis for new pulse_data
    patient_pulse_data.set_axis()
...
def set_axis():
    ...
    print "Patient bpm: " + str(patient_pulse_data.bpm)
...
def refresh_plot():
    global patient_waveform, start_idx, end_idx, s_idx, s, e_idx, e, patient_pulse_data
    #print "refresh_plot"
    # Clear previous plot
    a.clear()
    #print int(start_idx)
    ...
    #print int(s_idx[s])
    #print int(e_idx[e])
    y_min = min(patient_waveform[int(s_idx[s]):int(e_idx[e])])
    y_max = max(patient_waveform[int(s_idx[s]):int(e_idx[e])])
    #print int(y_min)
    #print int(y_max)
    ...
    #print type(y_min)
    #print type(y_max)
    a.axis([s_idx[s], e_idx[e], y_min, y_max])
...
def load_plot_var_func(arg1, arg2, arg3):
    print "load_plot_var_func"
#    print arg1
#    print arg2
#    print arg3
    load_plot_button.config(state='active')
...
def load_plot_button_func():
    global patient_pulse_data, fm, patient_waveform, s_idx, s, e_idx, e
    print "Attempting to load patient data..."
    if load_plot_var.get() == 'None':
        print "Please select patient"
    else:
        load_plot_button.config(state='disabled')
        zoom_in_button.config(state='active')
        patient_pulse_data = fm.load_all_data(load_plot_var.get())
        # Update current patient name
        patient_name.set(load_plot_var.get().replace("_", " "))
        #print patient_name.get()
        #print patient_pulse_data.bpm
        # Update current patient BPM
        bpm_value_label.config(text=str(patient_pulse_data.bpm))
        patient_waveform = patient_pulse_data.data_final
        # Adjust axis to new patient data
        set_axis()
        # Update plot with new patient data
        refresh_plot()

def save_patient_data_button_func():
    global patient_waveform, fm, patient_pulse_data
    ...
    fm.save_all_data(patient_name.get(), save_filename.get(), patient_pulse_data)
    # Update 'Load' OptionMenu list w/ newly saved Patient
#    load_plot_option['menu'].add_command(label=patient_name.get(), command=_setit(load_plot_var, patient_name.get()))
...
# END Constants
#left_frame = Frame(window, width=left_frame_width, height=left_frame_height,bg='#bcd6ff')
#left_frame = Frame(window, bg='#bcd6ff')
left_frame = Frame(window)
left_frame.pack(side=LEFT, expand=False, fill=X)
#left_frame.place(width=left_frame_width, height=left_frame_height)

## New Patient Information frame
info_frame = Frame(left_frame, width=left_frame_width, height=left_frame_height/2)
info_frame.pack(side=TOP, expand=False, fill=X)
...
# New Patient Information frame label
new_patient_label = Label(patient_info_frame, text="NEW PATIENT", bg='#d6ffc1')
new_patient_label.pack(side=TOP, fill=X)
...
measurement_frame.pack(side=TOP, fill=X)
# 'Start Measurement' Button
start_measure_button = Button(measurement_frame, text="Start Measurement", command=start_measure_button_func)
start_measure_button.pack(side=BOTTOM, fill=X)


# Load Old Patient Information frame
load_info_frame = Frame(left_frame, width=left_frame_width, height=left_frame_height/2)
load_info_frame.pack(side=TOP, expand=False, fill=X)

# Load Old Patient frame label
old_patient_label = Label(load_info_frame, text="LOAD PATIENT", bg='#c1fff6')
old_patient_label.pack(side=TOP, fill=X)

# 'Load Plot' button
load_plot_var = StringVar(load_info_frame)
load_plot_var.set("None")   # default value - no patient
load_plot_var.trace('w', load_plot_var_func)
load_plot_option = OptionMenu(load_info_frame, load_plot_var, *fm.patient_dir_names)
load_plot_option.pack(side=LEFT, fill=X)
load_plot_button = Button(load_info_frame, text="Load", command=load_plot_button_func)
load_plot_button.pack(side=RIGHT, fill=X)
load_plot_button.config(state='disabled')


## Patient Data Display frame
display_data_frame = Frame(left_frame)
display_data_frame.pack(side=BOTTOM, fill=X)

# Patient Data Display frame label
display_data_label = Label(display_data_frame, text="PATIENT DATA", bg='#ffd7c1')
display_data_label.pack(side=TOP, fill=X)

# Patient Name Frame
patient_name_frame = Frame(display_data_frame)
patient_name_frame.pack(side=TOP, fill=X)
# Patient Name Label
patient_name_label = Label(patient_name_frame, text="Patient Name: ")
patient_name_label.pack(side=LEFT)
# Patient Name Value
patient_name_value = Label(patient_name_frame, text="-")
patient_name_value.pack(side=RIGHT)

# Pulse Analysis Information frame
#   contains: BPM, directory name
analysis_info_frame = Frame(display_data_frame)
...
show_plot_button.config(state='disabled')
show_plot_button.pack(side=TOP, fill=X)
