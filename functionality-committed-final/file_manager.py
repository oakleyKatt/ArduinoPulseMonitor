...
class file_manager():
    ...
    # name @param should be formatted before passed
    def save_all_data(self, name, dir_name, this_pulse_data):
        # Check if name exists in database
        if self.check_name(dir_name) == 1:
            print "Directory name not taken"
            # Create directory for new Patient
            os.makedirs(os.getcwd() + "/Patient_Files/" + dir_name)
            print os.getcwd()+"/"+dir_name
            self.update_dir()
            # Get last entry from Patient Directory Path list
            n = self.patient_dir_names.index(dir_name)+1
            ...
            # Save Patient Information (name, directory name)
            patient_info = name
            self.save(self.patient_dir_path[n],"patient_info",patient_info)
        else:
    ...
    def load_all_data(self, name):
        ...
        data_clean = self.format_float(self.load(self.patient_dir_path[n]+'/data_clean.txt'))
        # Load data_filtered1 file
        data_filtered1 = self.format_float(self.load(self.patient_dir_path[n]+'/data_filtered1.txt'))
        # Load data_filtered2 file
        data_filtered2 = self.format_float(self.load(self.patient_dir_path[n]+'/data_filtered2.txt'))
        # Load data_final file
        data_final = self.format_float(self.load(self.patient_dir_path[n]+'/data_final.txt'))
        # Load data_info file
        data_info = self.format_float(self.load(self.patient_dir_path[n]+'/data_info.txt'))
        bpm_val = data_info[0]
        sample_period = data_info[1]
        return pulse_data(data_clean, data_filtered1, data_filtered2, data_final, bpm_val, sample_period)

    # Format unformatted data (from .txt file) as float
    def format_float(self, raw):
        x = []
